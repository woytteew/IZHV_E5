...
public class Character2DMovement : MonoBehaviour
{
    ...
    void AnimateCharacter()
    {
	    ...
	    if (mInput.move.x < 0)
	    {
		    if(mHeadingRight==true)	transform.RotateAround(transform.position, transform.up, 180f);
		    
		    mHeadingRight = false;
	    }
	    else if (mInput.move.x > 0)
	    {
		    if(mHeadingRight==false) transform.RotateAround(transform.position, transform.up, 180f);
		    mHeadingRight = true;
	    }
